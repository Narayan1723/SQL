...
--So here I change some columns data type
...
--So here I change columns data type
...
--first I select Total sales monthly wise
...
--calculate sales for rolling 3 month avg.
RollingAvg AS (SELECT Year,Month,total_sales,
AVG(total_sales) OVER (ORDER BY Year, MONTH ROWS BETWEEN 2 PRECEDING AND CURRENT ROW) 
AS 'rolling_3_month_avg'    --This calculates the average sales for the current month and the two previous months.
FROM MonthlySales)

-- So first I calculate sales diff. from current month and previous month to identifying growth or decline.
SELECT Year,Month,total_sales,rolling_3_month_avg,
total_sales - LAG(total_sales) OVER (ORDER BY Year, MONTH) 
AS 'sales_difference',      --LAG Function:Retrieves the sales amount from the previous month.
         --OVER: Defines the order in which rows are processed, so that LAG knows which row is the "previous" one.

-- and now I create a condition to identify trend
CASE 
...
--first I select profit margin product wise
...
--cumulative profit is the total profit accumulated over a specific period. 
...
SUM(profit_margin) OVER (PARTITION BY product_category ORDER BY profit_margin DESC) AS 'cumulative_profit'--Cumulative Profit = Sum of Profits
FROM ProductProfit)  --PARTITION BY divides the data into product_category wise
...
SELECT ST.store_id,ST.Store_name,P.Product_id,P.Product_name,SUM(S.units) AS 'total_units_sold',
I.Stock_on_hand AS 'stock_on_hand', 
CASE 
    WHEN I.Stock_on_hand > 0 
	THEN SUM(S.units) / avg(I.Stock_on_hand) --Inventory Turnover Ratio = Cost of Goods Sold / Average Inventory
    ELSE 0                             
...
LEFT JOIN Sales S      --LEFT JOIN Sales: Ensures all products in inventory are included.
